...
console.log('🚀 开始测试多用户节点拖拽协作功能...\n');

// 创建三个用户连接
const users = [
  { userId: 'dragger1', displayName: '拖拽者1', nodeId: 'node_root', color: '#ff6b6b' },
  { userId: 'dragger2', displayName: '拖拽者2', nodeId: 'node_text_1', color: '#4ecdc4' },
  { userId: 'observer1', displayName: '观察者1', nodeId: null, color: '#45b7d1' }
];
...
users.forEach((user, index) => {
  setTimeout(() => {
    console.log(`=== 🔗 创建用户 ${user.displayName} 的连接 ===`);
    ...
    ws.on('open', function open() {
      ...
      if (user.nodeId && user.nodeId !== 'node_root') {
        setTimeout(() => {
          ...
          setTimeout(() => {
            console.log(`🖱️ ${user.displayName} 开始拖拽节点 ${user.nodeId}...`);
            simulateDragOperation(ws, user);
          }, 2000);
        }, 4000);
      } else if (user.nodeId === 'node_root') {
        setTimeout(() => {
          console.log(`🖱️ ${user.displayName} 开始拖拽根节点...`);
          simulateDragOperation(ws, user);
        }, 5000);
      }
    });
    ...
    ws.on('message', function message(data) {
      try {
        ...
        if (message.type === 'online-users') {
          ...
          const operatorName = message.data.displayName || '未知用户';
          
          if (op.type === 'node-create') {
            console.log(`📦 ${user.displayName} 看到 ${operatorName} 创建了节点 ${op.nodeId}`);
            ...
            console.log(`${emoji} ${user.displayName} 看到 ${operatorName} ${status}节点 ${op.nodeId} 到位置 (${Math.round(op.data.position.x)}, ${Math.round(op.data.position.y)})`);
          }
        ...
        console.log(`❌ ${user.displayName} 消息解析失败:`, e.message);
      }
    });
...
function simulateDragOperation(ws, user) {
  ...
  if (user.userId === 'dragger1') {
    startX = 200; startY = 120; endX = 450; endY = 350;
  } else if (user.userId === 'dragger2') {
    startX = 400; startY = 250; endX = 150; endY = 400;
  }
...
setTimeout(() => {
  ...
  console.log('✅ 验证了NestJS WebSocket消息格式');
  console.log('✅ 测试了拖拽开始、进行中、结束的完整流程');
