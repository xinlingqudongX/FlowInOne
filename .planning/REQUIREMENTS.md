...
- [ ] **DATA-01**: 创建 `NodeMetadataEntity`，字段：nodeId (PK, 对应 LogicFlow node id)、requirement (TEXT)、prompt (TEXT)、attributes (JSON)、status (ENUM: pending/completed/failed/review_needed)、projectId (FK)
- [ ] **DATA-02**: 创建 `NodeExecutionHistoryEntity`，字段：id、nodeId (FK)、promptSnapshot (TEXT)、requirementSnapshot (TEXT)、result (TEXT)、executedAt、createdBy
- [ ] **DATA-03**: 生成并运行 MikroORM 迁移脚本，创建 `node_metadata` 和 `node_execution_history` 两张表
